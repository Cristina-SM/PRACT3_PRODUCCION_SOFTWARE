...
def test_create_multiple_expenses_and_list():
    ...
    service.create_expense("Pan", 3, "Mercado", date(2025, 1, 4))
    service.create_expense("Leche", 4, "Supermercado", date(2025, 1, 6))

    assert len(service.list_expenses()) == 2

    assert any(expense.title == "Pan" for expense in service.list_expenses())
    assert any(expense.title == "Leche" for expense in service.list_expenses())

    amounts = [expense.amount for expense in service.list_expenses()]
...
def test_remove_expense_reduces_total():
    ...
    service.create_expense("Libro", 20, "", date(2025, 1, 10))
    service.create_expense("Revista", 5, "", date(2025, 1, 15))
...
def test_update_expense_partial_fields():
    ...
    service.create_expense("Camiseta", 15, "Ropa", date(2025, 1, 20))
...
def test_total_amount_after_removal():
    ...
    service.create_expense("Cursos", 30, "", date(2025, 1, 25))
    service.create_expense("Internet", 25, "", date(2025, 1, 26))
